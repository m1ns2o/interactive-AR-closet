...
import type { VTONRequest, VTONResponse } from "../types/index.js";

export type ProgressCallback = (status: string, progress?: number, message?: string) => void;
...
export class VTONService {
  ...
  async processTryOn(
    ...
    try {
      ...
      const app = await client(this.spaceId, {
        hf_token: hfToken as `hf_${string}` | undefined,
      });

      const humanImageBlob = new Blob([request.humanImage], { type: "image/png" });
      const garmentImageBlob = new Blob([request.garmentImage], { type: "image/png" });
      ...
      const outputImage = typeof result.data[0] === 'object' && result.data[0] !== null
        ? (result.data[0] as any).url
        : result.data[0];

      const maskedImage = typeof result.data[1] === 'object' && result.data[1] !== null
